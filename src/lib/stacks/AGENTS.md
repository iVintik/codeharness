...
| python.ts | Python provider — app type detection, coverage (pytest-cov), OTLP packages, Dockerfile, test output parsing | `PythonProvider` |
| rust.ts | Rust provider — Cargo.toml parsing, coverage (tarpaulin), OTLP packages, multi-stage Dockerfile (rust:1.82-slim), workspace test aggregation | `RustProvider` |
