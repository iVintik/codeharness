...
export function configureWeb(projectDir: string, stack: string | null): void {
  const webInstaller: Record<string, () => void> = {
    nodejs: () => {
      try {
        ...
        // IGNORE: web OTLP packages are supplementary, failure is non-fatal
      }
    },
  };
  ...
  try {
    ...
    // IGNORE: state not readable, use default endpoint
  }
  ...
  const agentInstallers: Record<string, () => boolean> = {
    nodejs: () => {
      try {
        ...
        // IGNORE: agent Node.js OTLP packages are supplementary, failure is non-fatal
      }
      return true;
    },
    python: () => {
      try {
        ...
        // IGNORE: pip install failed, try pipx as fallback
        try {
          for (const pkg of AGENT_OTLP_PACKAGES_PYTHON) {
            execFileSync('pipx', ['install', pkg], { cwd: projectDir, stdio: 'pipe', timeout: 300_000 });
          }
        } catch {
          // IGNORE: agent Python OTLP packages are supplementary, failure is non-fatal
        }
      }
      return true;
    },
