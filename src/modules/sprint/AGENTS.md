...
- `selector.ts` — Story selection logic: `selectNextStory(state)` — prioritizes by tier (in-progress > verifying-with-proof > verifying > backlog), with tech debt gate within backlog tier (`TD-` prefixed stories selected before feature stories, per Decision 6c). Reports retry-exhausted stories. Exports `MAX_STORY_ATTEMPTS` constant.
...
- `budget.ts` — Sprint phase time budget awareness: `shouldDeferPhase(phase, remainingMinutes)` — returns true if session time is insufficient for the phase. `getPhaseEstimate(phase)` — returns estimated minutes. `computeRemainingMinutes(sessionStartMs, totalBudgetMinutes)` — computes remaining session time. Exports `PHASE_ESTIMATES` constant.
