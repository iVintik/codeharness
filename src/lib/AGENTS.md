...
| workflow-parser.ts | Workflow YAML parser — reads file, validates against JSON schema, resolves execution defaults and hierarchical flow semantics, and returns typed `ResolvedWorkflow`; re-exports workflow patch helpers for backward compat | `parseWorkflow`, `parseWorkflowData`, `WorkflowParseError`, `ResolvedWorkflow`, `ResolvedTask`, `LoopBlock`, `FlowStep`, `ExecutionConfig`, `HierarchicalFlow`, `BUILTIN_EPIC_FLOW_TASKS`, `EXECUTION_DEFAULTS`, `resolveHierarchicalFlow`, `resolveExecutionConfig`, `WorkflowPatch`, `resolveWorkflow` |
...
| workflow-compiler.ts | XState compiler — pure helpers that turn YAML gate/forEach/flow configs into XState state configs plus workflow-state helper utilities/constants | `compileStep`, `compileGate`, `compileForEach`, `compileFlow`, `HALT_ERROR_CODES`, `DEFAULT_MAX_ITERATIONS`, `PER_RUN_SENTINEL`, `isEngineError`, `isLoopBlock`, `buildRetryPrompt`, `buildAllUnknownVerdict`, `getFailedItems`, `recordErrorInState`, `isTaskCompleted`, `isLoopTaskCompleted` |
| workflow-error-utils.ts | Machine-layer error coercion helpers that translate dispatch/runtime failures into workflow engine errors | `handleDispatchError` |
| workflow-execution.ts | Referential integrity and flow validation helpers used during parsing; no longer owns the public hierarchical-flow API | `validateFlowReferences`, `validateReferentialIntegrity`, `ForEachBlock`, `GateBlock`, `ForEachFlowStep` |
