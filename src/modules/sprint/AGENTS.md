...
- `types.ts` — Module-specific types: `StorySelection`, `StoryDetail`, `StatusReport`, `SelectionResult`, `RetryExhaustedInfo`, `FailedStoryDetail`, `LabeledActionItem`, `RunSummary`, `StoryDrillDown`, `AcDetail`, `AttemptRecord`, `ProofSummary`, `TimeoutCapture`, `TimeoutReport`, `TimeoutSummary`, `FailingAc`, `FeedbackResult`
