...
- `index.ts` — Public API: `getSprintState`, `updateStoryStatus`, `getNextStory` (delegates to selector, marks retry-exhausted as blocked), `generateReport` (delegates to reporter)
- `reporter.ts` — Report generation: `generateReport(state)` — computes sprint progress, epic counts, run summary, failed story details, labeled action items. Pure function over SprintState, returns `Result<StatusReport>`.
...
- `types.ts` — Module-specific types: `StorySelection`, `StoryDetail`, `StatusReport`, `SelectionResult`, `RetryExhaustedInfo`, `FailedStoryDetail`, `LabeledActionItem`, `RunSummary`
